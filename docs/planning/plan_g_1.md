...
n_splits=1` (verified, sklearn 1.9.0). The clamp above reaches 1 exactly when
overall groups or minority class row counts force it. If `requested < 2`, there is 
no sensible grouped-CV fold count, so the run must fail with a named, actionable error
(`InsufficientGroupsError`) reporting the distinct group count and row count, 
rather than crashing inside sklearn.

**Empty test folds are mathematically normal.** In the per-class-group-sparsity case, 
a minority class can have plenty of rows but trace back to only 1-2 distinct 
`seed_prompt_id`s. `StratifiedGroupKFold` does **not** raise here, and instead 
naturally emits empty *test* folds for that class in some splits. This is mathematically 
expected (Pigeonhole Principle) and harmless. Empty test folds do not corrupt the 
training process. The model trains normally on the folds where the class is present 
in the training set. A fold where the class is missing from the *training* half will 
trigger the single-class degenerate head branch, which is tolerated by design and 
made visible by `constant_head_fold_count` (or fatal via `--strict-folds`). Clamping 
the global fold count down to 2 just to avoid empty test folds would drastically 
degrade the overall CV training ratio for all hazards and is not mathematically sound.
...
**`no_segments` is not a neutral input, and the docs must explain its context-dependent nature.** A zero
feature vector is not "no signal" once it reaches the model: the standardizer
maps it to `(0 - mean) / scale`. Whether this represents an extrapolation to an 
out-of-distribution point or a well-calibrated score depends on the judgment.

For `legitimizing`, nothing filters sentences, so a zero feature row can only arise 
from a response with no segments at all — a case training drops. For `legitimizing`, 
the resulting score is an **extrapolation to an out-of-distribution point**, not a 
calibrated judgment. 

For `enabling`, `effective_indices` discards prompt-repetition sentences that have 
no later authored continuation (`scoring_common.py:307-309`), so an enabling row 
whose response is *entirely* echoed prompt text keeps zero effective indices and 
`build_response_matrix` feeds it `zero_feature` (`:366-369`) as a **training** row. 
Therefore, for `enabling`, the zero vector is **not out-of-distribution**, and the 
model's score is explicitly learned and well-calibrated.

Consequences, all required:

- `docs/production_howto.md` states plainly that for `legitimizing`, `no_segments: true` rows carry
  a model score that should not be trusted as a graded judgment. But for `enabling`, the score is 
  well-calibrated and can be trusted.
...
- `train` emits `zero_feature_train_row_count` per judgment in `cv_metrics.json` to monitor this behavior.
...
- `test_cv_seed_prompt_grouping.py` — **the regression guard for the confirmed
  ...
  3. **(CV-3) per-class group sparsity** — a minority class whose rows all trace
     ...
     `actual_folds` equals `folds` (no clamping) and the run completes normally. 
     Some test folds will correctly and harmlessly have empty counts for the rare class.
  4. **(CV-4)** exactly **1** overall distinct `seed_prompt_id` or exactly **1** 
     row for a minority class, asserting the run raises `InsufficientGroupsError` 
     naming the issue — the fold-floor guard (G4), which without an explicit `< 2` 
     check would instead surface as sklearn's opaque `n_splits=1` `ValueError`;
...
flag for rather than trust as a graded judgment** for `legitimizing`, while clarifying
that for `enabling` the score is well-calibrated (since prompt-repetition-only rows put
zero-feature vectors in the training set); the embedder-identity guard and what its error means; a scripted smoke
