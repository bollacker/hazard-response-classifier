...
Numbering: new decisions start at **D-78**. D-38 through D-44 were drafted
...
<a id="d-77"></a>
## D-77: PR 5's approved-criteria exit criterion is discharged by scoping, and what replaces it
Date: 2026-08-05
Status: locked
Approved by: Kurt, 2026-08-05, on the recommendation `PR5_EXECUTION_PLAN.md`
§3 carried as gate G-3. **Riki's concurrence assumed on Kurt's direction, not
confirmed on record.**

Decision: `RELEASE_1_1_QUEUE_PROPOSAL.md` PR 5's first exit criterion — "the
selected models meet approved per-outcome criteria on evaluation rows excluded
from fitting" — **cannot be met in Release 1.1 and is met by scoping rather
than by building.** What replaces it, and what PR 5 delivers instead: a
per-outcome report over the held-out dev slice — L0, L1, L2, E0, E1, E2
separately, each with precision, recall, and F1, each carrying a cluster
bootstrap interval and the method that produced it — with **both models
reported *not evaluated*** whatever the figures show.
`RELEASE_1_1_QUEUE_PROPOSAL.md` PR 5's exit criteria carry the change;
`PR5_DEV_METRICS.md` and `pr5_results/dev_metrics.json` are the deliverable,
generated by `scripts/report_le_dev_metrics.py`.

Rationale: the criterion has two halves and only one is buildable. **"Meet
approved per-outcome criteria"** requires criteria that do not exist:
`STANDARDS_REQUEST.md` Ask B asks the Standards team to set them, and
[D-63](#d-63) established that their inputs are not arriving. Approved
criteria are a **policy judgment about acceptable risk**, not a measurement —
no dataset, no model, and no amount of engineering substitutes for one, which
is precisely why `SCIENCE.md` §Legitimization Scoring and §Enablement Scoring
say to report the model as *not evaluated* in their absence rather than to
invent a threshold. **"On evaluation rows excluded from fitting"** *is*
buildable and is met: [D-73](#d-73) fits the shipped artifact on the fit half
alone, so the dev slice is genuinely excluded and every reported number
describes the model that ships.

So the criterion is not being waived — it is being split, with the half that
depends on nobody else met in full and the half that depends on an absent
policy input recorded as unmet, with its replacement named. This is the same
shape as [D-54](#d-54) and [D-55](#d-55) for PR 4, and the same reason: a
release closes honestly by recording which of its stated criteria it did not
meet, never by quietly restating them as something it did.

**What the replacement is not.** The dev-slice report is not a benchmark
result, not a generalization estimate, and not a weaker form of the criterion
partially satisfied. Under [D-66](#d-66) it is dev-class; under D-63 the labels
are out-of-version; under [D-65](#d-65) coverage is attacked prompts only; and
[D-68](#d-68) is a null result. A reader who treats the report as evidence the
models are good enough has done exactly what this entry exists to prevent, so
the report states that beside its numbers rather than beneath them.

Rejected alternatives: (1) **setting per-outcome criteria in this repository
so the criterion could be met** — rejected as the failure the whole
not-evaluated rule guards against: criteria written by the people who built
and measured the model, after seeing its numbers, are not approved criteria
and calling them one would make every downstream claim unfalsifiable;
(2) **deleting the criterion from PR 5's exit list** — rejected because it
destroys the record that Release 1.1 ships without approved criteria, which is
the single most important thing D-47's limitations disclosure has to say;
(3) **blocking PR 5's close until Ask B arrives** — rejected because D-63
established it is not arriving, so this makes the release unclosable rather
than more rigorous, and stalls exactly the way `META_PLAN.md` exists to
prevent; (4) **reporting a single headline figure instead of per outcome** —
rejected because `SCIENCE.md` requires all three outcomes be treated as
equally important and a macro hides a collapsed class, which on this data it
would have: L2's recall is 0.222 behind a macro-F1 of 0.491.

Touches: `RELEASE_1_1_QUEUE_PROPOSAL.md` PR 5's exit criteria (**absorbed** —
the criterion is struck, the scoping stated, and the replacement named);
`PR5_EXECUTION_PLAN.md` §3's G-3 and §10's exit-criteria table;
`PR5_DEV_METRICS.md`, `pr5_results/dev_metrics.json`,
`scripts/report_le_dev_metrics.py`; `README.md` §Release 1.1 evaluator status,
which carries the summary; [D-47](#d-47)'s limitations inventory, which
already records "no approved success criteria exist" and gains nothing new
here.

Boundary: this discharges **one exit criterion of one PR**. It does not make
either model evaluated, does not weaken `SCIENCE.md` §Evidence and outputs'
not-evaluated rule, and does not license reporting any dev-slice figure as a
quality claim. It does not touch PR 5's other four exit criteria, which are
met by building. It does not retire `STANDARDS_REQUEST.md` Ask B: if approved
